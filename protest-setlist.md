---
title: ""
mainfont: Times New Roman
documentclass: extarticle
fontsize: 14pt
geometry: a5paper, top=1cm, left=1cm, right=1cm, bottom=1cm
classoption: portrait
output: pdf
header-includes:
  - \usepackage{fontspec}
  - \usepackage{xcolor}
  - \newfontfamily\armystencil{ARMY RUST}
  - \newfontfamily\sansfont{Arial} 
  - \pagestyle{empty}
---

```{=latex}
\begin{center}
{\armystencil\fontsize{52pt}{12pt}\selectfont Protest Days,\\Rebellion Nights}

\vspace{3 em}
\end{center}

\begin{tabbing}
00.00 \= WELCOME - \itshape no phones / see you on the other side
\\
00.45 \> [ WEAVING SONG ]
\\
02.30 \> NED LUDD
\\
06.30 \> TOPHAM'S TALE \\ \itshape ... and that is the tale of Benjamin Topham
\\
11.00 \> [ CROPPER SONG INTRO MUSIC ]
\\
\> HUDDERSFIELD CROPPERS
\\
14.45 \> [ CROPPER SONG V1 + C ]
\\
\>SMALL ATTACKS
\\
15.30 \> [ CROPPER SONG V2 + C ]
\\
\>GUNS 
\\
16.25 \>  [ CROPPER SONG V3 + C ]
\\
\> MILL SELECT, DUMB STEEPLE
\\
17.45 \> [ CROPPER SONG V4 + 2 x C ]
\\
18.15 \> ATTACK ON CARTWRIGHT'S MILL — BRONTE
\\
\> JOHN BOOTH AND SAMUEL HARTLEY
\\
\> \itshape ... and so can I.
\\
25.00 \> [ CROPPER SONG V + C REPRISE (quiet) ]
\\
26.15 \> FUNERAL — COURT MARTIAL — 
\\
\> WHIPPING — MURDER PLAN — 
\\
\> HORSFALL MAN HABIT — CLOTH HALL
\\ \> \itshape ... Business done, on horse, started to make way home
\end{tabbing}

\newpage
\begin{center}
{\armystencil\fontsize{52pt}{12pt}\selectfont Protest Days,\\Rebellion Nights}

\vspace{3em}
\end{center}

\begin{tabbing}

30.00 \= [ JOHN THE WEAVER MUSIC INTRO ]
\\
\> PUB — DRINK — \itshape starts to have a drink...
\\
30.30 \> [ JOHN THE WEAVER 3 x VERSES (MUTTON CHOP) ]
\\
\> \itshape Horsfall starts up hill
\\
\> [ JOHN THE WEAVER V4 ]
\\
\> \itshape There's a shot, and another, and Horsfall cries "MURDER"
\\
\> [ JOHN THE WEAVER V5 ]
\\
\> DEATH OF HORSFALL — OATHS
\\
\> PROCLAMATION — 66 MEN — TRIAL
\\
\> GEO. MELLOR / WILL. THORPE / THOS. SMITH
\\
\> 14 MORE HANGED, OTHERS TRANSPORTED
\\
\> ONTO PLATFORM — ENEMIES/FORGIVE AS WORLD
\\
41.00 \> [ FAREWELL ME LADS SONG ]
\\
43.30 \> NOT JUST THE LUDDITES
\\
45.00 \> ONE MORE STORY, ONE MORE SONG
\\
\> MARCH OF THE BLANKETEERS
\\
\> \itshape One man made it to London. Waiting there still? 
\\
53.00 \> [ PLUG DRAWERS' MUSIC INTRO ]
\\
\> NOT END M/CR MARCHES — PLUG DRAWERS
\\
\> [ PLUG DRAWERS' SONG ]
\\
56.00 \> END
\end{tabbing}
```

